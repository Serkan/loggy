...
}}else{if(G.transport=="sse"){if(!K()){I("ObservableResource Side Events(SSE) is not supported, using request.fallbackTransport ("+G.fallbackTransport+")")
}else{A(false)
...
}if(!G.enableProtocol){if(!au){F("opening","sse",G)
...
}if(!G.enableProtocol){if(!au){F("opening","websocket",G)
...
at=false;
if(aw.transport!="long-polling"){ah(aw)
}}else{ah(aw)
}return at
}function Y(at,au){ae();
O.state="error";
...
while(au!=-1){ay=jQuery.trim(ax.substring(ay,au));
...
var ay=false;
var aC=false;
if(jQuery.browser.opera&&at.transport=="streaming"&&at.readyState>2&&au.readyState==4){ae();
aw();
return
...
aw();
return
}var aA=au.responseText;
if(jQuery.trim(aA.length)==0&&at.transport=="long-polling"){if(!au.hasData){aw()
...
O.reason=at==0?"ObservableResource resumed the connection or down.":"OK";
var aw=(av.connectTimeout==-1)?0:av.connectTimeout;
if(!ax){av.id=setTimeout(function(){l(av)
},aw)
}else{l(av)
...
var aw=function(az){var aB=jQuery.xdr.responseText;
if(au.lastMessage==aB){return
...
var aG=aH.innerText;
aG=jQuery.text.substring(0,aG.length-1);
return aG
};
