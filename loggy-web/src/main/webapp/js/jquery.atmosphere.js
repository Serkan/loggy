        ...
        AtmosphereRequest: function (options) {
            ...
            function _execute() {
                ...
                if (_request.transport != 'websocket' && _request.transport != 'sse') {
                    ...
                    if (!_supportSSE()) {
                        _reconnectWithFallbackTransport("ObservableResource Side Events(SSE) is not supported, using request.fallbackTransport (" + _request.fallbackTransport + ")");
                    } else {
                        _executeSSE(false);
                    }
                }
            ...
            function _reconnect(ajaxRequest, request, force) {
                if (force || request.transport != 'streaming') {
                    if (request.reconnect || (request.suspend && _subscribed)) {
                        ...
                        _response.reason = status == 0 ? "ObservableResource resumed the connection or down." : "OK";
